...
(`orderHash 0xb4eceb58…3325`) cites `qpu_job_id d9loj33hdfks73cl9in0` — the
...
**A third finding, and it did not survive contact with a second instance.**
The reps=2 collapse above is the extreme form of a pathology that survives at
reps=3 in milder form: tuned on ⟨H⟩, the circuit's most-likely feasible state
on the replication instance is the **7th-best portfolio of 56**, on a noiseless
simulator, before any hardware noise. Swapping the classical tuner's objective
to **CVaR_α** (the mean of the best α probability mass, Barkoutsos et al.,
*Quantum* **4**, 256 (2020)) fixed that on the instance it was measured on —
and then failed to reproduce on the next one. We report both.

**Instance A — the replication window** (mu/sigma in
`outputs/replication_marrakesh_interleaved.json`), exact statevector:
...
That result is robust *within* instance A: swept over seeds 1/7/42/123/2024/
31337, ⟨H⟩ converges on the same rank-7 portfolio at all six and CVaR on the
optimum at all six. So it is a property of the objective on that instance, not
a seed artefact.

**Instance B — a matched pair run on real hardware, 2026-08-17.** Same eight
tickers, same budget 3, same risk factor 0.5, same reps=3 ring circuit, same
backend (`ibm_marrakesh`), both arms submitted minutes apart against the same
market window. The only difference is the tuner objective. **CVaR loses on
every column, on the simulator and on the QPU:**

| tuner objective | sim P(opt) | hw raw P(opt) | hw mitigated P(opt) |
|---|---|---|---|
| **⟨H⟩ (mean)** | **0.2305** | **0.0264** | **0.0186** |
| CVaR α=0.50 | 0.1865 | 0.0112 | 0.0164 |

Job IDs, with raw counts in the artefacts: `da1q6o4dedkc73ercci0` /
`da1q6q6g52gs73clvj6g` (mean, `outputs/hardware_run_mean_20260817.json`) and
`da1q58qein7c73bdo9g0` / `da1q5b4dedkc73ercb20` (CVaR,
`outputs/hardware_run_cvar0p5.json`). These are the **first CVaR-tuned QPU runs
in this project**, and they are a negative result.

**What we therefore claim: nothing about CVaR being better.** Two instances,
one each way, is not evidence of an effect — it is evidence that the objective
choice is instance-dependent on this problem family, and that the instance-A
gain was not the general property the first write-up of it implied. `mean`
remains the default everywhere and every shipped artefact is mean-tuned.
`--objective cvar` stays in the codebase as a switch whose result is recorded
in the artefact, not as a recommendation. Establishing which objective wins,
and when, needs a paired sweep across many windows — the same n ≥ 10 design the
mitigation question needs, and the same funded work.

**The α mechanism is still the one the paper describes**, and it is worth
keeping because it is checkable independently of which objective wins.
Barkoutsos et al. note that α "introduces a soft cap on the maximum probability
of sampling a ground state… because the CVaR objective function with α = 1%
does not reward increasing the overlap with the ground state beyond 1 %
probability." The instance-A numbers land on that cap almost exactly: α=0.25
yields P(optimal) = 0.2500 and α=0.10 yields 0.1036. That is a reproduction of
a published effect. It also explains why α=0.5 was chosen over the
α ∈ [0.1, 0.25] the paper recommends — their range optimises convergence of the
variational loop under a sampling-accuracy argument, we were optimising
P(optimal) at a fixed 4 096 shots — but it does not predict which objective wins
on a given instance, and instance B is the demonstration that it does not.

**One thing this never meant.** The pipeline does not decode modally.
`_score_counts` scans every sampled feasible bitstring and keeps the
lowest-objective one, so a mean-tuned run at 4 096 shots still reports the
optimum — every arm in both tables above returned the exact optimum
(-0.8681). What the tuning objective moves is P(optimal), the quantity that
decides whether a run clears the uniform-over-feasible null of 1/C(8,3); it was
never the difference between a right answer and a wrong one.

**The simulator figures in this section — 0.2078, 0.2195, 0.1848, 0.2305 — are
different market windows, not competing measurements of one number.** Each
comparison above is internally controlled; none is comparable across rows.
...
|---|---|---|---|
| 1. Anchor the **routing** commitment | AuditAnchorV2 | [`0xfc44e9d4…b3b1`](https://monadscan.com/tx/0xfc44e9d4828d10a9e05444227cc0496df84451f6ef8ffc08822da8b0dad5b3b1) | sequence 0; orderHash `0xb4eceb58…3325` = SHA-256 of a hedged ML-DSA + SLH-DSA + Ed25519 signed order, with a commitment binding the vault, chain, token, fee tier, weights, amount, slippage floor and deadline |
| 2. `executeAndRoute(0.1 MON → USDC)` | UniswapRoutingVault | [`0xa229f4a6…edfc6`](https://monadscan.com/tx/0xa229f4a6dc0b7421cca1cfa1e988cba30722dba0862677f4e8a75375e5cedfc6) | routed **0.1 MON → 2 125 USDC micro-units** (`Routed.amountsOut[0] = 0x84d`) through the live WMON/USDC 0.3% pool [`0x659bD0BC…4a9da`](https://monadscan.com/address/0x659bD0BC4167BA25c62E05656F78043E7eD4a9da), above the signed floor of **2 046**; zero WMON dust; status success |
...
`0xb4eceb58…3325` — see "Now live on Monad mainnet" above and the reviewer
...
**measured 1 192 307 gas** (`attest()` tx `0x4cc8290b…c5e4`)
...
`0xb4eceb58…3325` — the same order anchored, swapped and Morpho-supplied
...
| `attest()` tx | [`0x4cc8290b…c5e4`](https://monadscan.com/tx/0x4cc8290b5338b388750b7ea3b00ae47ebd0af86a9a5d38f1096e1337f70dc5e4) — Groth16 proof verified on-chain (1 192 307 gas); `PQOrderAttested(0xb4eceb58…)` emitted; `pqAttested[orderHash] == true` |
...
measured 8.1M-gas cost of verifying ML-DSA natively in the EVM. Measured replacement: 1 192 307 gas, a 6.8x reduction (4.1x against the optimised 4.9M ETHDilithium variant).
...
**The limitation the gate did not close — now closed, on mainnet.** The ZK
guest commits `(orderHash, pkHash)` and nothing more, so the proof establishes
*"the pinned ML-DSA key signed some order hashing to X"* — not *what X
authorises*. The execution binding lived in `AuditAnchorV2.execCommitmentOf`,
whose value is chosen by whoever calls `anchor()`. `AlreadyAnchored` is keyed
per-caller, so any address — including the agent's own ECDSA key — could bind
an arbitrary execution to an already-attested orderHash and execute it,
producing an on-chain record that passed every check while describing a trade
the signature never authorised.

It was never theft: the caller funds `msg.value` and the router pays out to
`msg.sender`. The damage was to provenance, in a system whose product is the
audit trail — and it defeated the two-key custody property this repository
asserts elsewhere, under the exact threat model that property names (ECDSA key
compromised, post-quantum key not).

**The fix, shipped 2026-08-10.** Schema-v3 orders carry `exec_commitment` — a
keccak256 over the execution parameters — *inside the post-quantum-signed
bytes*. The executor is handed the order preimage and
`contracts/src/PQExecBinding.sol` (1) checks `sha256(preimage) == orderHash`,
so the preimage really is the order the attestation covers, (2) reads
`exec_commitment` out of it, and (3) recomputes the same hash from its own
calldata and compares. A compromised ECDSA key cannot alter step 2's value
without breaking step 1 — which is precisely the threat model this system
declares. The order hash is deliberately *not* part of `exec_commitment`:
`orderHash = sha256(canonical bytes)` and those bytes now contain the
commitment, so including it would make the two mutually recursive. Step 1
binds the order directly instead.

`contracts/test/redteam/RT12_ExecCommitmentUnbound.t.sol` used to **pass while
asserting the attack succeeded**. Its four tests now assert that every attack
path reverts, including the sharper one where the agent's own ECDSA key tries
to diverge from post-quantum-signed intent. Binding cost, measured: 102,718
gas on a 120-byte preimage.

**Deployment status, stated plainly: this is live on mainnet.** The previous
`UniswapRoutingVault` `0x06F233…1e56` and `MorphoSupplyAdapter`
`0x8d5AE2…E49E` are immutable with no owner and no upgrade path — by design —
so the gate could not be added to them. Shipping it meant deploying new
...
> submission's attested `orderHash 0xb4eceb58…` from their own address with an
...
from the chain alone that the execution which ran is the execution the
post-quantum signature authorised — the executor refuses anything else. You no
longer need `outputs/signed_orders.json` to establish that binding, though the
artefact remains reproducible and `verify_claims.py` still recomputes it.
Honest scope unchanged: no quantum advantage anywhere; this is the
PQ-*settlement* path, proven end to end.
...
"
# → orderHash      = 0xb4eceb5893e4d68181706a8c3e08631cb24409880d1ccbd053082d1550a03325
# → execCommitment = 0xe0a6ab379cd9f78d6df64b86eec5a899b0e9c4c2f6177761bdfb207b72ace116
...
# → 0xe0a6ab379cd9f78d6df64b86eec5a899b0e9c4c2f6177761bdfb207b72ace116
...
cast receipt 0xfc44e9d4828d10a9e05444227cc0496df84451f6ef8ffc08822da8b0dad5b3b1 \
  --rpc-url https://rpc.monad.xyz | grep -E "^(status|blockNumber|gasUsed)"
# → blockNumber  100349618
# → gasUsed      76555
# → status       1 (success)
...
**5. Prove the binding is REAL rather than described.** Replay the execution
that settled, with one field changed, and watch mainnet refuse it. Read-only,
costs nothing, needs no key — and needs no archive node.

Both arms run against HEAD on the public endpoint. There is no `--block` pin
because there is nothing historical to reach: both answers come out of contract
storage, so neither depends on a live price nor on an endpoint still retaining
state it has since pruned.

That is a deliberate correction, made twice. The first version of this
demonstration ran at HEAD against an order whose `deadline` then passed — the
vault checks `DeadlinePassed` at `UniswapRoutingVault.sol:210`, *before* the
commitment check at 231, so both arms collapsed into the same revert and the
contrast proved nothing. The second pinned block 99252441 to dodge that, and
`https://rpc.monad.xyz` pruned that height within days, answering
`-32602: Block requested not found` to the very commands a reviewer was told to
run. The order below carries `deadline 1956528063` (2031-12-31) and has already
executed, which is what makes both refusals permanent instead of dated.
...
H=0xb4eceb5893e4d68181706a8c3e08631cb24409880d1ccbd053082d1550a03325
U=0x8dF64bACf6b70F7787f8d14429b258B3fF958ec1
SIG="executeAndRoute(bytes32,address[],uint24[],uint16[],uint256[],uint256,bytes)"

# A. THE SIGNED EXECUTION — every field exactly as post-quantum signed:
cast call $V "$SIG" $H "[0x754704Bc059F8C67012fEd69BC8A327a5aafb603]" \
  "[3000]" "[10000]" "[2649]" 1956528063 "$PRE" \
  --from $U --value 100000000000000000 --rpc-url https://rpc.monad.xyz
# -> execution reverted, data: "0x1162d898b4eceb58..."
#    AnchorAlreadyConsumed(orderHash). It got PAST the commitment check and was
#    stopped by single-use replay protection — this exact order really settled,
#    in tx 0xa229f4a6…dfc6.

# B. TAMPERED — identical call, floor 2649 -> 1, the deviation a compromised
#    ECDSA key would want:
cast call $V "$SIG" $H "[0x754704Bc059F8C67012fEd69BC8A327a5aafb603]" \
  "[3000]" "[10000]" "[1]" 1956528063 "$PRE" \
  --from $U --value 100000000000000000 --rpc-url https://rpc.monad.xyz
# -> execution reverted, data: "0x1f9e3c96e70f4990..."
#    ExecCommitmentMismatch(signed, recomputed). It never reached replay
#    protection: one changed field, and the commitment check refuses an
#    execution the signature never authorised.
```

The two reverts differ, and the difference is the evidence. The untampered call
clears the commitment check and dies at the consumed guard; the tampered one
dies at the commitment check itself. `verify_claims.py --chain` runs both of
these commands on every pass and fails if either stops answering.
...
**448 tests** (`pytest tests/ && forge test`). Nothing skips when
...
**267 Python tests**
- `test_verify_claims.py` (126) — tests OF the claim gate: every retired figure
...
- **Area 3 (primary) — Digital Infrastructure Secured Against Quantum
  ...
  **267 Python tests** (32 PQ-signing, 36 Monad-TX encoding, 18 live-quote,
  17 key-pinning/hedge-policy, 17 key-rotation, 8 audit-chain, 126 verifier
  ...
  448 tests, none skipped once the fork environment is set. The artefacts are tamper-evident and a reviewer can
  ...
  attestation share `orderHash 0xb4eceb58…3325`; the yield leg carries its
...
avoid it is cost, not feasibility: our Groth16 check is 1 192 307 gas, 6.8x
cheaper than the NIST-compliant native verifier.

**That is the gap we closed.** `MLDSAAttestation` verifies a Groth16 proof of an
ML-DSA-65 signature on Monad mainnet for a **measured 1 192 307 gas — 0.8 % of
...
value, 448 tests passing with none skipped once the fork environment is set,
...
circuit, and 448 passing tests. That is the argument for a pilot, and the
...
`0xb4eceb58…3325`, and `AuditAnchorV2.execCommitmentOf` on Monad **mainnet**
