...
| 4 | **Streamlit — optimizer** | 9 s | "Here's the agent picking Markowitz-optimal weights, then running the same circuit on IBM Heron hardware. Single run per arm — we report the confidence intervals, and they overlap." *(26 words, ~9 s)* |
...
| 7 | **Proof — live on Monad mainnet, 235 tests** | 10 s | "Live on Monad mainnet, all Monadscan-verified. Two hundred thirty-five tests across Python and Foundry. The post-quantum signature is verified on-chain by a zero-knowledge proof. Reproducible from a cold clone." *(20 words, ~7 s)* — leave 3 s of silence for the numbers to land |
