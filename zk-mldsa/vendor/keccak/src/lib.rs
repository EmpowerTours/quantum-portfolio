...
impl Keccak {
    ...
    pub fn with_p1600<const ROUNDS: usize>(&self, f: impl FnOnce(Fn1600)) {
        // SP1 acceleration: for the standard full Keccak-f[1600] (24 rounds),
        // call the SP1 keccak-permute precompile instead of the pure-Rust
        // permutation. This is the hot path for ML-DSA's SHAKE, and moves it
        // off the RISC-V core, cutting cycles and prover memory dramatically.
        #[cfg(all(target_os = "zkvm", target_vendor = "succinct"))]
        if ROUNDS == F1600_ROUNDS {
            fn sp1_keccak_f1600(state: &mut State1600) {
                unsafe {
